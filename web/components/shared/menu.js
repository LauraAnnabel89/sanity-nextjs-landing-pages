      ...
      <ul {...props}>
        {items.map((item, i) => {
          if (item._type === "reference") {
            ...
            if (route.route) {
              if (route.route.toPath({ slug: resolved.slug.current }) === router.asPath)
                className = "menu__item--current";
            }
              ...
              return (
                <li key={i} className={className}>
                  <Link route={_type} href="/about">
                    <a onClick={onClick}>{resolved.title || resolved.name}</a>
                  </Link>
                </li>
              );
            }
          ...
          let className;
          if (item.url === router.asPath || router.asPath.indexOf(item.url) === 0)
            className = "menu__item--current";

          return (
            <li key={i} className={className}>
              <NextLink href={item.url}>
                <a onClick={onClick}>{item.name || item.url}</a>
              </NextLink>
            </li>
          );
        })}
