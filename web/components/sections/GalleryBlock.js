    ...
    <figure className={`wp-block-gallery columns-${cols} is-cropped`}>
      ...
      {isOpen && (
        <Lightbox
          ...
          nextSrc={node.images[(photoIndex + 1) % node.images.length].asset.url}
          prevSrc={
            node.images[(photoIndex + node.images.length - 1) % node.images.length].asset.url
          }
          onCloseRequest={() => setIsOpen(false)}
          onMovePrevRequest={() =>
            setPhotoIndex((photoIndex + node.images.length - 1) % node.images.length)
          }
          onMoveNextRequest={() => setPhotoIndex((photoIndex + 1) % node.images.length)}
          enableZoom={false}
          // imageCaption={`Camera: ${
